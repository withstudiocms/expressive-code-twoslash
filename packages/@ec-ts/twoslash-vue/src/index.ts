/** biome-ignore-all lint/suspicious/noExplicitAny: This is part of the Source API */
/** biome-ignore-all lint/style/noNonNullAssertion: This is part of the Source API */
import type {
...
export function createTwoslasher(createOptions: CreateTwoslashVueOptions = {}): TwoslashInstance {
	...
	function getVueLanguage(
		...
		function getLanguage() {
			...
			return createLanguage(
				[vueLanguagePlugin],
				new FileMap(ts.sys.useCaseSensitiveFileNames) as unknown as Map<
					string,
					SourceScript<string>
				>,
				() => {},
	...
	function twoslasher(code: string, extension?: string, options: TwoslashVueExecuteOptions = {}) {
		if (extension !== "vue") return twoslasherBase(code, extension, options);
		...
		const { customTags = createOptions.customTags || [] } = options;
		...
		const sourceScript = lang.scripts.set(
			`${ts.sys.getCurrentDirectory()}/index.vue`,
			ts.ScriptSnapshot.fromString(strippedCode),
		)!;
		...
		if (createOptions.debugShowGeneratedCode) return result;

		// Map the tokens
		const mappedNodes = result.nodes
			.map((q) => {
				if ("text" in q && q.text === "any") return undefined;
				const startMap = get(map.toSourceLocation(q.start), 0);
				if (!startMap) return undefined;
				...
				if (end == null || start < 0 || end < 0 || start > end) return undefined;

/** biome-ignore-all lint/suspicious/noExplicitAny: This is okay */
...
// Create require function for ESM compatibility with Node.js modules
const require =
	typeof createRequire !== "undefined" && typeof import.meta !== "undefined"
...
export function createVirtualTypeScriptEnvironment(
	...
	return {
		...
		createFile: (fileName, content) => {
			// biome-ignore lint/style/noNonNullAssertion: This is a public API, we want it to be as easy to use as possible, and the error message when this fails is pretty clear about what went wrong.
...
export const knownLibFilesForCompilerOptions = (compilerOptions: CompilerOptions, ts: TS) => {
	...
	const getMax = (array: number[]) =>
		array?.length ? array.reduce((max, current) => (current > max ? current : max)) : undefined;
...
export const createDefaultMapFromNodeModules = (
	...
	const getLib = (name: string) => {
		const lib = tsLibDirectory || path.dirname(resolveModule("typescript"));
		return fs.readFileSync(path.join(lib, name), "utf8");
	};

	const isDtsFile = (file: string) => /\.d\.([^.]+\.)?[cm]?ts$/i.test(file);

	const libFiles = fs.readdirSync(tsLibDirectory || path.dirname(resolveModule("typescript")));
...
export function createFSBackedSystem(
	...
	const tsLib = tsLibDirectory ?? path.dirname(resolveModule("typescript"));
					...
					save(
						ts.createSourceFile(
							...
							languageVersionOrOptions ??
								...
								// biome-ignore lint/style/noNonNullAssertion: This is a public API, we want it to be as easy to use as possible, and the error message when this fails is pretty clear about what went wrong.
								defaultCompilerOptions(ts).target!,
							false,
						),
					)
				);
...
const requirePath = () => {
	if (!require) {
		throw new Error("Node.js 'path' module is not available in this environment");
	}
	return require("node:path") as typeof import("path");
};
...
const requireFS = () => {
	...
	return require("node:fs") as typeof import("fs");
};

const resolveModule = (moduleName: string) => {
	if (!require) {
		throw new Error("Node.js 'require.resolve' is not available in this environment");
	}

...
export function validateCodeForErrors(
	...
	const unspecifiedErrors = relevantErrors.filter(
		(e) => e.code && !handbookOptions.errors.includes(e.code as number),
	);
	...
	if (unspecifiedErrors.length) {
		...
		if (noFiles.length) innerDiags.push(showDiagnostics("Ambient Errors", noFiles));

...
import { flagKeys } from "./flag-keys.ts";
import { reAnnonateMarkers, reConfigBoolean, reConfigValue } from "./regexp.ts";
import type { TwoslashReturnMeta } from "./types/index.ts";
import { findCutNotations } from "./utils.ts";
...
export function removeTwoslashNotations(code: string, customTags?: string[]): string {
	...
	const tags = [...(customTags ?? []), ...flagKeys];

	Array.from(code.matchAll(reConfigBoolean)).forEach((match) => {
		if (!tags.includes(match[1])) return;

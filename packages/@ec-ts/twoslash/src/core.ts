/** biome-ignore-all lint/style/noNonNullAssertion: This is the design of the Source API */
/** biome-ignore-all lint/suspicious/noExplicitAny: This is the design of the Source API */
import { createFSBackedSystem, createSystem, createVirtualTypeScriptEnvironment } from "@ec-ts/vfs";
import type {
	ErrorLevel,
	NodeError,
	NodeWithoutPosition,
...
import { defaultCompilerOptions, defaultHandbookOptions } from "./defaults.ts";
import { TwoslashError } from "./error.ts";
import type {
...
import { validateCodeForErrors } from "./validation.ts";
...
export function createTwoslasher(createOptions: CreateTwoslashOptions = {}): TwoslashInstance {
	...
	const system = useFS
		? createSystem(vfs)
		: createCacheableFSBackedSystem(
				vfs,
				_root,
				ts,
				createOptions.tsLibDirectory,
				createOptions.fsCache,
			);
	const fsRoot = useFS ? "/" : `${_root}/`;

	const cache =
		createOptions.cache === false
			? undefined
	...
	function getEnv(compilerOptions: CompilerOptions) {
		if (!cache)
			return createVirtualTypeScriptEnvironment(
				...
				ts as any,
				compilerOptions,
				createOptions.customTransformers,
			);
	...
	function twoslasher(
		...
		const isInRemoval = (index: number) =>
			index >= code.length || index < 0 || isInRanges(index, meta.removals, false);
		...
		function getIdentifiersOfFile(file: VirtualFile) {
			if (!identifiersMap.has(file.filename)) {
				...
				identifiersMap.set(
					file.filename,
					getIdentifierTextSpans(ts, source, file.offset - (file.prepend?.length || 0)),
				);
			}
		...
		function getQuickInfo(
			...
			if (quickInfo?.displayParts) {
				...
				const tags = quickInfo.tags?.map(
					(t) => [t.name, t.text?.map((i) => i.text).join("")] as [string, string | undefined],
				);
		...
		for (const file of meta.virtualFiles) {
			...
			if (
				supportedFileTyes.includes(file.extension) ||
		...
		if (!meta.handbookOptions.showEmit) {
			for (const file of meta.virtualFiles) {
				...
				if (!meta.handbookOptions.noStaticSemanticInfo) {
					...
					for (const [start, _end, target] of identifiers) {
						...
						if (!shouldGetHoverInfo(target, start, file.filename)) continue;
			...
			for (const query of meta.positionQueries) {
				...
				if (id) node = getQuickInfo(file, id[0], id[2]);
			...
			for (const target of meta.positionCompletions) {
				...
				if (prefix) {
					...
					prefix =
						(completions[0]?.replacementSpan &&
							code.slice(completions[0].replacementSpan.start, target)) ||
						prefix;
				...
				// If not, we try to trigger with character (e.g. `.`, `'`, `"`)
					...
					if (prefix) {
						...
						if (completions[0]?.replacementSpan?.length) {
							prefix = code.slice(completions[0].replacementSpan.start, target) || prefix;
							const newCompletions = completions.filter((i) => i.name.startsWith(prefix));
							if (newCompletions.length) completions = newCompletions;
						}
					}
				}
		...
		for (const file of meta.virtualFiles) {
			...
			if (meta.handbookOptions.noErrors !== true) {
				...
				for (const diagnostic of diagnostics) {
					if (diagnostic.file?.fileName !== file.filepath) continue;
					if (ignores.includes(diagnostic.code)) continue;
					const start = diagnostic.start! + file.offset - (file.prepend?.length || 0);
					if (meta.handbookOptions.noErrorsCutted && isInRemoval(start)) continue;
					// @ts-expect-error -- this is fine
					errorNodes.push({
						type: "error",
						start,
		...
		if (meta.handbookOptions.showEmit) {
			...
			if (!meta.handbookOptions.keepNotations) {
				...
				for (const file of files) env.updateFile(file.filepath, getFileContent(file));
			}
			...
			let emitSource = meta.virtualFiles.find(
				(i) => removeTsExtension(i.filename) === removeTsExtension(emitFilename),
			)?.filename;
			...
			if (meta.compilerOptions.outFile) emitSource = meta.virtualFiles[0].filename;

			const output = ls.getEmitOutput(fsRoot + emitSource);
			const outfile = output.outputFiles.find(
				(o) => o.name === fsRoot + emitFilename || o.name === emitFilename,
			);
		...
		// cleanup
		for (const file of meta.virtualFiles) env.createFile(file.filepath, "");
		for (const file of Object.keys(extraFiles)) env.createFile(fsRoot + file, "");
...
function createCacheableFSBackedSystem(
		...
		? {
				...
				sys: {
					...
					readFile(path, encoding) {
						if (encoding === undefined) return cachedReadFile(path);
	...
	return {
		...createFSBackedSystem(vfs, root, cachedTs as any, tsLibDirectory),

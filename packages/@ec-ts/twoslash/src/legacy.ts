...
import type { HandbookOptions, TwoslashExecuteOptions, TwoslashReturn } from "./types/index.ts";
...
export function convertLegacyOptions<T extends TwoslashOptionsLegacy>(
	opts: T,
...
export function convertLegacyReturn(result: TwoslashReturn): TwoslashReturnLegacy {
	return {
		...
		staticQuickInfos: result.hovers.map((i): TwoslashReturnLegacy["staticQuickInfos"][0] => ({
			text: i.text,
			docs: i.docs || "",
			start: i.start,
			length: i.length,
		...
		tags: result.tags.map((t): TwoslashReturnLegacy["tags"][0] => ({
			name: t.name,
			line: t.line,
			...(t.text ? { annotation: t.text } : {}),
		})),

		highlights: result.highlights.map((h): TwoslashReturnLegacy["highlights"][0] => ({
			kind: "highlight",
		...
		queries: (
			[
				...result.queries.map((q): TwoslashReturnLegacy["queries"][0] => ({
					kind: "query",
					docs: q.docs || "",
					offset: q.character,
					start: q.start,
				...
				...result.completions.map((q): TwoslashReturnLegacy["queries"][0] => ({
					kind: "completions",
					offset: q.character,
					start: q.start,
					length: q.length,
		...
		errors: result.errors.map((e): TwoslashReturnLegacy["errors"][0] => ({
			id: e.id ?? "",

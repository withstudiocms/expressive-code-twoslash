...
import { defaultHandbookOptions } from "./defaults.ts";
...
export function getIdentifierTextSpans(
...
export function parseFlag(
	...
	const compilerDecl = tsOptionDeclarations.find(
		(d) => d.name.toLocaleLowerCase() === name.toLocaleLowerCase(),
	);
			...
			case "list": {
				...
				const resolved =
					typeof elementType === "string"
						...
						: strings.map((v) =>
								getOptionValueFromMap(compilerDecl.name, v, elementType as Map<string, string>),
							);
				return {
					type: "compilerOptions",
					name: compilerDecl.name,
	...
	if (Object.keys(defaultHandbookOptions).includes(name)) {
		...
		if (name === "errors" && typeof value === "string") value = value.split(" ").map(Number);

		// "noErrors" can be a boolean or a list of numbers
		if (name === "noErrors" && typeof value === "string") {
			if (value === "true") value = true;
			else if (value === "false") value = false;
			else value = value.split(" ").map(Number);
		}
...
export function findCutNotations(code: string, meta: Pick<TwoslashReturnMeta, "removals">) {
		...
		if (comment.match(reCutBefore)) {
			...
			if (startIdx === undefined) {
				const startLine = lines.findIndex(
					(line, i) => i > lineIndex && line.trim().match(reCutStart),
				);
	...
	if (meta) meta.removals.push(...removals);
...
export function findQueryMarkers(
	...
	if (code.includes("//")) {
		...
		Array.from(code.matchAll(reAnnonateMarkers)).forEach((match) => {
			...
			while (linesQuery.has(targetLine) && targetLine >= 0) targetLine -= 1;
			...
			if (type === "?") {
				...
				meta.positionHighlights.push([targetIndex, targetIndex + markerLength, match[2]?.trim()]);
			}
